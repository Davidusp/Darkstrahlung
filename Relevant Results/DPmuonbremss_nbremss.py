...
def main():
    ...
    NDPs_tot = np.zeros(N) #Number of DPs generated in each angle, step and energy
                ...
                if ((700/np.cos(theta[t])-x[j]/2.65/100)>=0.0):
                    ...
                    for r in range(0, N):
                        ...
                        if (r!=0):
                            ...
                            dsig_dx[i][r] = diff_cross_section(epsilon, alpha, mDP, E[i][j]/1000, m_mu, Z, (ratio[i][r]+ratio[i][r-1])/2)
    ...
    with open('/home/dfreitas/DPnum_Comp/Bremss/Num_energy/NBremss_m=%.13f_k=%.11f.txt' %(mDP,epsilon), 'w') as archive:
        for i in range(0, len(NDPs_tot)):
            archive.write('%.11f    %.20f\n' %(E_z[i], NDPs_tot[i]))
main()