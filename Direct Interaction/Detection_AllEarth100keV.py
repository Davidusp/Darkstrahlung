...
def MuonCalc(Ei, theta, E, mDP, x, kappas, Nmuabove100, Na):
    ...
    #DP energies (GeV)
    if (mDP*1e-03<=1E-06):
        E_z = np.linspace(1E-06, 0.0002, N) #GeV

    elif (mDP*1e-03>1e-06 and mDP*1e-03<=1e-03):
        E_z = np.linspace(mDP*1e-03, 0.0002, N) #GeV
...
def CalcEvents(NDPs_tot, theta, mDP, kappas, Na, epsilon, funcmantle, funcore, R_earth):
    ...
    Ndec_res = np.zeros((len(theta), 19500, N)) #Number of decays in each angle, step and energy in Earth
    Ndec_tot = np.zeros((len(theta), 19500, N)) #Number of decays in each angle, step and energy in crystals
    ...
    for t in range(0,len(theta)): #angles
        ...
        if d >= R_core:
            ...
            x_m1 = np.linspace(0,L_m,6500)
            x_c = np.linspace(L_m, (L_m+L_c),6500)
            x_m2 = np.linspace((L_c+L_m),(L_c+2*L_m),6500)
...
def CalcOsc(theta, NDPs_tot, kappas, mDP, Na, funcNaI):
    ...
    xdetect = np.linspace(0,0.4/np.cos(detectheta[len(detectheta)-1]),300) #Steps inside the NaI crystals (m)
...
def main():
    ...
    # mDP = 0.02559500 #MeV
    # kappas = np.array([0.00002738420,0.00004216965,0.00006493816,
    #                    0.00010000000,0.00015399265,0.00023713737,0.00036517413,0.00056234133,0.00086596432,
    #                    0.00133352143,0.00205352503,0.00316227766,0.00486967525,0.00749894209,0.01154781985,
    #                    0.01778279410,0.02738419634,0.04216965034,0.06493816316,0.10000000000])
    ...
    theta_earth = np.linspace(3.0*np.pi/180,177*np.pi/180,55) # Earth's theta angle in spherical coordinates, considering n degrees resolution
        
    #print(theta*180/np.pi)
    en=np.logspace(0,3,4)
    interen=np.zeros(4)
    Ei=np.logspace(3,6.69897,900) #Muon energies from 1 GeV to few TeV (in MeV)
    ...
    x=np.linspace(0,14000*2.65*100,3400) # Define step length in cm used in the muon attenuation and DP generated by Bremsstrahlung in the rock.
    ...
    #         archive.write('%.16f\n' %(np.sum(NDPs_tot[:,i])))
    # with open('/home/davidc/Documents/Master\'s Analisys/Parameter space/Codes/Direct Interaction/NDPstot100keV_m=%.8f_k=%.11f.txt' %(mDP,kappas), 'w') as archive:
    #     for i in range(0, len(NDPs_tot[0])):
    #         archive.write('%.16f\n' %(np.sum(NDPs_tot[:,i])))
    ...
    # if mDP < 2*0.511:
    #     with open('/home/davidc/Documents/Master\'s Analisys/Parameter space/Codes/Direct Interaction/CompleteSpec/SpecAllEarth/100keV/NDir100keV_int_m=%.8f_k=%.11f.txt' %(mDP,kappas), 'w') as archive:
    #         for i in range(0, N):
    #             archive.write('{:.11f}  {:.16f}  {:.16f}  {:.16f}\n'.format(E_z[i], NdecNaI[i], NOscNaI[i], Ndec[i]))

    # else:
    #     with open('/home/davidc/Documents/Master\'s Analisys/Parameter space/Codes/Direct Interaction/CompleteSpec/SpecAllEarth/100keV/NDir100keV_int_int_m=%.8f_k=%.11f.txt' %(mDP,kappas), 'w') as archive:
    #         for i in range(0, N):
    #             archive.write('{:.11f}  {:.16f}  {:.16f}\n'.format(E_z[i], NdecNaI[i], Ndec[i]))

    #if mDP < 2*0.511:
    with open('/data/COSINE/WORK/dfreitas/single_hit/Spectra100keV/NDir100keV_int_m=%.8f_k=%.11f.txt' %(mDP,kappas), 'w') as archive:
        for i in range(0, N):
            archive.write('{:.11f}  {:.16f}  {:.16f}  {:.16f}\n'.format(E_z[i], NdecNaI[i], NOscNaI[i], Ndec[i]))

    # else:
    #     with open('/home/dfreitas/DPnum_Comp/DetecAllEarth/Spectra100keV/NDir100keV_int_m=%.8f_k=%.11f.txt' %(mDP,kappas), 'w') as archive:
    #         for i in range(0, N):
    #             archive.write('{:.11f}  {:.16f}  {:.16f}\n'.format(E_z[i], NdecNaI[i], Ndec[i]))
