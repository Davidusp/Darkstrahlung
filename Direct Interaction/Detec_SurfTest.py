...
def CalcEvents(Ei, theta, E, mDP, x, kappas, Nmuabove100, Na, epsilon, funcrock, funcNaI):
    ...
    if (mDP*1e-3>1E-06):
        E_z = np.linspace(mDP*1e-3, 0.1, N) # DP energy (GeV)
                    
    else:
        E_z = np.linspace(1E-06, 0.1, N) #DP energy (GeV)
                    ...
                    if r!= 0:
                        ...
                        if j>0:
                            ...
                            if np.sum(NDPs_tot[t][:j+1,r]) < np.sum(NAbs_res[t][:j,r]):
                                ...
                                exit()
                                NAbs_res[t][j][r] -= np.abs(round(np.sum(NAbs_res[t][:j,r]), 4)-round(np.sum(NDPs_tot[t][:j+1,r]), 4))
                            ...
                            if np.sum(NDPs_tot[t][:j+1,r]) < np.sum(NOsc_res[t][:j,r]):
                                ...
                                exit()
                                NOsc_res[t][j][r] -= np.abs(np.sum(NOsc_res[t][:j,r])-round(np.sum(NDPs_tot[t][:j+1,r]),3))
                ...
                for r in range(N):
                    ...
                    if j>0:
                        ...
                        if np.sum(NDPs_tot[t][:j+1,r]) < np.sum(NComps_tot[t][:j,r]):
                            ...
                            exit()
                            #NComps_tot[t][j][r] -= np.abs(round(np.sum(NComps_tot[t][:j,r]), 2)-round(np.sum(NDPs_tot[t][:j+1,r]), 2))
                        ...
                        if np.sum(NDPs_tot[t][:j+1,r]) < np.sum(NAbs_tot[t][:j,r]):
                            ...
                            exit()
                            #NAbs_tot[t][j][r] -= np.abs(round(np.sum(NAbs_tot[t][:j,r]), 2)-round(np.sum(NDPs_tot[t][:j+1,r]), 2))
                        ...
                        if np.sum(NDPs_tot[t][:j+1,r]) < np.sum(NOsc_tot[t][:j,r]):
                            ...
                            exit()
                            #NOsc_tot[t][j][r] -= np.abs(np.sum(NOsc_tot[t][:j,r])-round(np.sum(NDPs_tot[t][:j+1,r]),3))
                        ...
                        if NDPs_tot[t][j][r] < NComps_tot[t][j][r]:
                            ...
                            exit()
                            #NComps_tot[t][j][r] -= np.abs(round(NComps_tot[t][j][r], 2)-round(NDPs_tot[t][j][r], 2))    
                        ...
                        if NDPs_tot[t][j][r] < NAbs_tot[t][j][r]:
                            ...
                            exit()
                            #NAbs_tot[t][j][r] -= np.abs(round(NAbs_tot[t][j][r], 1)-round(NDPs_tot[t][j][r], 1))
                        ...
                        if NDPs_tot[t][j][r] < NOsc_tot[t][j][r]:
                            ...
                            exit()
                            #NOsc_tot[t][j][r] -= np.abs(NOsc_tot[t][j][r]-round(NDPs_tot[t][j][r],3))
                        NDPs_res[t][r] =- (NComps+NAbs+NOsc)
...
def main():
    ...
    # with open('/home/davidc/Documents/Master\'s Analisys/Parameter space/Codes/Direct Interaction/NDir_int_m=%.8f_k=%.11f.txt' %(mDP,kappas), 'w') as archive:
    #     for i in range(0, N):
    #         archive.write('{:.11f}  {:.16f}  {:.16f}  {:.16f}\n'.format(E_z[i], NCompsNaI[i], NAbsNaI[i], NOscNaI[i]))
        
    with open('/home/dfreitas/DPnum_Comp/DirDetec/CompleteSpec/NDir_int_m=%.8f_k=%.11f.txt' %(mDP,kappas), 'w') as archive:
        for i in range(0, N):
            archive.write('{:.11f}  {:.16f}  {:.16f}  {:.16f}\n'.format(E_z[i], NCompsNaI[i], NAbsNaI[i], NOscNaI[i]))
