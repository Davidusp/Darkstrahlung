...
def Calc_kq(E_k, kpre, k, kabs, costheta, qpre, q, qabs, m_e, m_phi):
    for m in range(len(E_k)):
        print(m)
        i=0
        eps_DP = (E_k[m]**2-m_phi**2)**0.5*0.001
        while (i<10000):
            kpre[0]=np.random.normal(0,((E_k[m]**2-m_phi**2)**0.5)/3)
            kpre[1]=np.random.normal(0,((E_k[m]**2-m_phi**2)**0.5)/3)
            kpre[2]=np.random.normal(0,((E_k[m]**2-m_phi**2)**0.5)/3)
        ...
        costheta[m]=np.linspace(-1,1,10000)

        i=0
        while (i<10000):
            qpre[0]=np.random.normal(0,(m_phi**2+2*E_k[m]*m_e)/(3*2*(m_e+E_k[m]-kabs[m][i]*costheta[m][i])))
            qpre[1]=np.random.normal(0,(m_phi**2+2*E_k[m]*m_e)/(3*2*(m_e+E_k[m]-kabs[m][i]*costheta[m][i])))
            qpre[2]=np.random.normal(0,(m_phi**2+2*E_k[m]*m_e)/(3*2*(m_e+E_k[m]-kabs[m][i]*costheta[m][i])))
            qpreabs=(qpre[0]**2+qpre[1]**2+qpre[2]**2)**0.5

            eps_q = (m_phi**2+2*E_k[m]*m_e)/(2*(m_e+E_k[m]-kabs[m][i]*costheta[m][i]))*0.01
            eps_cos = 0.005
...
def main():
    ...
    m_e = 0.511  #MeV/c²
    m_phi = 0.3  #MeV
    E_k = np.logspace(m_phi+0.01*m_phi,np.log10(100),100) # DP energy (MeV/c²)

    #k = sqrt(E_k²-m_phi²)
    kpre = np.zeros(3)
    k = np.zeros((len(E_k),10000,3))
    kabs = np.zeros((len(E_k),10000))

    qpre = np.zeros(3)
    q = np.zeros((len(E_k),10000,3))
    qabs = np.zeros((len(E_k),10000))

    pp=np.zeros((len(E_k),10000,3))
    ppabs=np.zeros((len(E_k),10000))

    costheta = np.zeros((len(E_k),10000))

    p=[0.0,0.0,0.0] 
    dsig_dcos=np.zeros((len(E_k),10000))
