...
def main():
    ...
    if m_phi < 2*m_e:
        ...
        E_k = np.logspace(np.log10(m_phi+0.001*m_phi),np.log10(1e6),80) # DP energy (MeV/c²)
    ...
    for m in range(len(E_k)):
        ...
        Ne=11 #Rock
        #Ne = 0.466*8+0.2772*14+0.0813*13+0.05*26+0.0363*20+0.0283*11+0.0259*19+0.0209*12+0.0141*22
    ...
    # with open('/home/dfreitas/DPnum_Comp/Comp-xs2/xs1MeVplus/CSxE_m=%.13f_k=%f.txt' %(m_phi, kappa), 'w') as archive:
    #     for i in range(0, len(E_k)):
    #         archive.write('%.11f   %.16f\n' %(E_k[i], sig[i]))
    
    # with open('/home/davidc/Documents/Master\'s Analisys/Results Compton/Cross Section 2/CSxE_m=%.13f_k=%f.txt' %(m_phi, kappa), 'w') as archive:
    #     for i in range(0, len(E_k)):
    #         archive.write('%.11f   %.16f\n' %(E_k[i], sig[i]))
    ...
    plt.grid(True)
    plt.show()
    #plt.savefig("/home/davidc/Documents/Master\'s Analisys/Results Compton/Cross Section 2/CS_%dE_%dcos_k=%f_mphi=%f_Rock.png" %(len(E_k), len(q), kappa, m_phi))
