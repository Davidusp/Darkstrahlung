...
def CalcAbs(theta, mDP, x, Na, kappas, NDPs_tot, E):
    '''
    ...
    NAbs_res = np.zeros((len(theta), len(x))) #Number of absorption processes in each angle and step
    ...
    E_k = np.logspace(np.log10(1e-3),np.log10(100), len(x)) #MeV
    ...
    for t in range(0,len(theta)):
        for j in range(0,len(x)-1):
            ...
            if ((700/np.cos(theta[t])-x[j]/2.65/100)>=0.0):
                ...
                #EDITAR DAQUI PRA BAIXOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
                if j>0:
                    NAbs = (np.sum(NDPs_tot[t][:j+1]) - np.sum(NAbs_res[t][:j]))*2.65*Na/22*np.diff(x)[0]/2.65*sigAbs[len(sigAbs)-1-j]*1e-24 # num of absorptions per year
                    
                    if j==3370 and t== 14:
                        print("sig = %f    totalDPs = %f    totalAbs = %f" %(sigAbs[len(sigAbs)-1-j], np.sum(NDPs_tot[t][:j+1]), np.sum(NAbs_res[t][:j])))

                    # if math.isnan(np.sum(NAbs_res[t][:j])):
                    #     print("j = %d   t = %d   sig = %f    totalDPs = %f    totalAbs = %f" %(j, t, sigAbs[len(sigAbs)-1-j], np.sum(NDPs_tot[t][:j+1]), np.sum(NAbs_res[t][:j-1])))
                    #     exit()

                    NAbs_res[t][j] += NAbs

                else:
                    NAbs = NDPs_tot[t][j]*2.65*Na/22*np.diff(x)[0]/2.65*sigAbs[len(sigAbs)-1]*1e-24 # num of absorptions per year
                    NAbs_res[t][j] += NAbs
                    #print("Inital", NAbs, sigAbs[len(sigAbs)-1])
...
def main():
    ...
    NAbs_res = CalcAbs(theta, mDP, x, Na, kappas, NDPs_tot, E)

    # with open('/home/davidc/Documents/Master\'s Analisys/Parameter space/Codes/DPtot_Comp_m=%f_k=%.11f.txt' %(mDP,kappas), 'w') as archive:
    #     totalDPs = NDPs_tot - NAbs_res
    #     archive.write('%.11f   %.16f\n' %(kappas, np.sum(totalDPs)))

    # with open('/home/dfreitas/DPnum_Comp/Absorption/Absnumber/DPtot_Abs_m=%.8f_k=%.11f.txt' %(mDP,kappas), 'w') as archive:
