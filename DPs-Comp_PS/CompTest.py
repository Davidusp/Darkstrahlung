...
def CalcEvents(Ei, theta, E, mDP, x, kappas, Nmuabove100, Na, epsilon):
    '''
    ...
    NDPs_tot = np.zeros((len(theta), len(x), N)) #number of DPs generated in each angle and step
    NComps_res = np.zeros((len(theta), len(x), N)) #Number of absorption processes in each angle and step
    ...
    # with open('/home/davidc/Documents/Master\'s Analisys/Results Compton/Cross Section 2/CSxE_m=%f_k=%s00.txt' %(mDP, epsilon), 'r') as archive:
    #         arq = archive.read()  
    ...
    for l in range(0, len(lines)):
        E_k[l] = lines[l][0]
        sigComp[l] = (13/64)*lines[l][1]*(1/2.74e22)*(5.06e13)**2 #GeV⁻²/atom
    ...
    for t in range(0,len(theta)): #angles
        ...
        for j in range(0,len(x)-1): #steps inside rock
            ...
            if ((700/np.cos(theta[t])-x[j]/2.65/100)>=0.0):
                ...
                sigComp = (kappas/epsilon)**2*CScompfunc(1e3*E_z) #Absorption cross section
                ...
                for r in range(0, N):
                    if r!= 0:
                        ...
                        if j>0:
                            ...
                            NComps = (round(DPshere, 4))*(1 - np.exp(-L*5.06e13/(lamb)))  #num of Comptons/year for this DP energy
                            NComps_res[t][j][r] += NComps

                        else:
                            lamb = 1/(ne*sigComp[r])
                            NComps = NDPs_tot[t][j][r]*(1 - np.exp(-L*5.06e13/(lamb)))  #num of Comptons/year for this DP energy
                            NComps_res[t][j][r] += NComps
...
def main():
    ...
    NComps_res = CalcEvents(Ei, theta, E, mDP, x, kappas, Nmuabove100, Na, epsilon)

    # with open('/home/davidc/Documents/Master\'s Analisys/Parameter space/Codes/DPtot_Comp_m=%f_k=%.11f.txt' %(mDP,kappas), 'w') as archive:
    #     archive.write('%.11f   %.16f\n' %(kappas, np.sum(NComps_res)))

    with open('/home/dfreitas/DPnum_Comp/Compton/Num_right/NComp_m=%.13f_k=%.11f.txt' %(mDP,kappas), 'w') as archive:
        archive.write('%.11f   %.16f\n' %(kappas, np.sum(NComps_res)))
